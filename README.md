...
4. Create a `.env` file in the `backend/` folder and add your OpenRouter API key: `OPENROUTER_API_KEY=your_api_key_here`. Run the backend with `fastapi dev`
5. Create a `.env` file in the `frontend/` folder and add the backend URL: `FAST_API_BASE_URL=http://localhost:8000`
6. In `frontend/app/` folder, run the frontend with `streamlit run main.py`
